...
def create_image(request):
    if request.method == 'POST':
        ...
        if image_form.is_valid():
            ...
            # save returns image object with commit false with out saving to DB
            new_image = image_form.save(commit=False)
            # assign current user to the item
            new_image.user = request.user
            # now save the new image to the database
            new_image.save()
        ...
        # if get request, build form with data provided by the bookmarklet via Get
...
def images_list(request):
    ...
    try:
        ...
        if images_only:
            # if page and ajax request is out of range,
            # return an empty image
            return HttpResponse('')
        # if page out of range, return an empty image
